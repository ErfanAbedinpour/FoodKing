...
export class GenerateNewTokensHandler implements ICommandHandler<GenerateNewTokensCommand, { accessToken: string, refreshToken: string }> {
    ...
    async execute(command: GenerateNewTokensCommand): Promise<{ accessToken: string, refreshToken: string }> {
        try {
            const tokenPayload = await this.jwtService.verifyAsync<IRefreshTokenPayload>(command.refreshToken, { secret: process.env.REFRESH_TOKEN_SECRET });
