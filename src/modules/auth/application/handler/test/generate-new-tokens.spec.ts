...
describe("CreateUserUseCase", () => {
    ...
    it("Should be Unauthorized because Token is invalid", async () => {
        jest.spyOn(jwtServiceMock, 'verifyAsync').mockRejectedValue(new JsonWebTokenError("error"));
        try {
            const res = await generateNewTokensHandler.execute({ refreshToken: "fake-token" } as GenerateNewTokensCommand)
    ...
    it("Should be Unauthorized because Token is valid but invalidated IN DB", async () => {
        jest.spyOn(jwtServiceMock, 'verifyAsync').mockResolvedValue({ tokenId: "token-id" });
        jest.spyOn(userSessionRepository, 'isValidate').mockResolvedValue(false);

        try {
            const res = await generateNewTokensHandler.execute({ refreshToken: "valid-token" } as GenerateNewTokensCommand)
            expect(res).toThrow(UnauthorizedException)
            expect(res).toThrow(ErrorMessage.INVALID_REFRESH_TOKEN)
    ...
    it("should be Throw BadRequest if User Not Found", async () => {
        jest.spyOn(jwtServiceMock, 'verifyAsync').mockResolvedValue({ tokenId: "token-id", userId: 1 });
    ...
    it("Should be return execute Successfully", async () => {
        jest.spyOn(jwtServiceMock, 'verifyAsync').mockResolvedValue({ tokenId: "token-id", userId: 1 });
        jest.spyOn(userSessionRepository, 'isValidate').mockResolvedValue(true);
        jest.spyOn(userService, 'findById').mockResolvedValue({ id: 1, name: "test-name", role: { name: "Customer" } } as User)
        jest.spyOn(commandBusMock, 'execute').mockResolvedValueOnce({ accessToken: "access", refreshToken: "refresh" })


        const res = await generateNewTokensHandler.execute({ refreshToken: "valid-token" } as GenerateNewTokensCommand)
        expect(res).toEqual({ accessToken: "access", refreshToken: "refresh" })
        expect(commandBusMock.execute).toHaveBeenCalledWith({ userId: 1, name: "test-name", role: "Customer" })
        expect(userService.findById).toHaveBeenCalledWith(1)
        expect(userSessionRepository.isValidate).toHaveBeenCalledWith('token-id')
        expect(userSessionRepository.invalidate).toHaveBeenCalledWith('token-id')
    })
