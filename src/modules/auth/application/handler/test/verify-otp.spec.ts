...
describe('verifyOTP', () => {
  ...
  it('Should be throw BadRequest for Invalid Token', () => {
    ...
    const promise = verifyOtpUseCase.execute({
      code: '',
      phone: '',
    } as VerifyOtpCommand);
  ...
  it('Should be verified successful', async () => {
    ...
    jest
      .spyOn(otpRepository, 'findOtp')
      .mockReturnValueOnce({ code: '12341234', exp: Date.now() * 2 });
    ...
    const res = await verifyOtpUseCase.execute({
      code: '12341234',
      phone: '0911',
    } as VerifyOtpCommand);
