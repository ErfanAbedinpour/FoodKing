...
describe("CreateUserUseCase", () => {
    ...
    it("should throw BadRequest because Phone is exist before", async () => {
        jest.spyOn(userService, 'findByPhone').mockResolvedValueOnce({} as unknown as User)
    ...
    it("Should be register user successful", async () => {
        ...
        const input = { email: 'test-email', name: "test-name", password: "test-pass", phone_number: "test-phone" }
        jest.spyOn(userService, 'findByPhone').mockResolvedValueOnce(null)
        jest.spyOn(userService, 'createUser').mockResolvedValueOnce()

        const result = await createUserUseCase.execute({ ...input, phone: input.phone_number })
