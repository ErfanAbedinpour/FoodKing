...
describe("MikroMenuRepository", () => {
    ...
    beforeEach(async () => {
        for (let i = 1; i <= 10; i++) {
            const payload: MenuPersist = {
                en_title: `menu-${i}`,
                slug: `/slug-${i}`,
                sub_menus: [{ en_title: `first-${i}`, slug: `/first-${i}`, title: `first` }],
                title: `title-${i}`
            }
    ...
    it("Should be create Menu and subs", async () => {
        const payload: MenuPersist = {
            en_title: "menu-en",
            slug: "/menu",
            sub_menus: [{ en_title: "first", slug: "/first", title: "first" }, { en_title: "second", slug: "/second", title: 'second' }],
            title: "menu"
        }
    ...
    it("Should be throw Repository Error Because Menu not found", async () => {
        ...
        expect(mikroRepository.update(25, {})).rejects.toThrow(ErrorMessage.MENU_NOT_FOUND)
    })
